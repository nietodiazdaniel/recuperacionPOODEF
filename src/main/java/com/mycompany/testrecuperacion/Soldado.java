...
public class Soldado extends HumanoCombatiente {
    ...
    public void moverse(Tablero tablero, Casilla posicion,Juego juego){
        ...
        if (esContiguaHorizontalmente || esContiguaVerticalmente) {
            ...
            juego.getPantallaJuego().agregarEvento("El humano soldado se ha movido a la posicion "  + nueva.getCoordenada().toString());
        }
        else {
            juego.getPantallaJuego().agregarEvento("El humano soldado no se puede mover porque esta rodeado de zombies, utiliza la accion en otra accion diferente a moverse");
        }
    ...
    public void atacar(Tablero tablero,Juego juego){
        ...
        if(casillaTablero.getNumZombie().get(0).getNumHeridas()<5){
            ...
            juego.getPantallaJuego().agregarEvento("El humano soldado ha hecho 1 herida a "+casillaTablero.getNumZombie().get(0).getNombre()+" por lo que tiene "+casillaTablero.getNumZombie().get(0).getNumHeridas()+" heridas");
            ...
            if(casillaTablero.getNumZombie().get(0).getNumHeridas()==5){
                ...
                juego.getPantallaJuego().agregarEvento("El humano soldado ha matado al zombie "+casillaTablero.getNumZombie().get(0).getNombre());
